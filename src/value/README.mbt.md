...
value produces the same hash on every call); dict/set operations probe a key's hash more than
once without caching it, so a non-deterministic hash can produce duplicate logical entries.
