...
All optional features (`set`, `lambda`, `while`, `bytes`, `float`) are **enabled by default**. Recursive function calls are **disabled by default** (matching starlark-go); enable them with `Options::with_allow_recursion(true)`.
