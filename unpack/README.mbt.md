...
| :--- | :--- |
| `unpack_args` | Bind positional + keyword args to a name spec (`"name"`, `"name?"`, `"name??"`); returns the bound values as `Array[@value.Value?]` |
| `unpack_positional` | Bind positional-only args, requiring between `min` and `max` values; returns the bound values as `Array[@value.Value?]` |
| `unpack_args_with` | Like `unpack_args` but dispatches each value to a custom `@value.Unpacker` target; returns `Unit` because each target consumes its own value |
